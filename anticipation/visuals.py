...
def visualize(tokens, output, selected=None):
    ...
    instruments = sorted(list(ops.get_instruments(tokens).keys()))
    instruments = [instr for instr in instruments]
    ...
    patches = [matplotlib.patches.Patch(color=colors[i+1], label="{l}".format(l=instruments[i]))
